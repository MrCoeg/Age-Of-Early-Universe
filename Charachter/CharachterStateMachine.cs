...
public class CharachterStateMachine : StateMachine
{
    ...
    public bool Talking()
    {
        if (playerInput.Input.Talking && ableToTalk)
        {
            ChangeStateHandler(new TalkingState(GetComponent<DialogDetection>().loader));
            return true;
        }
        return false;
    }
    ...
    [SerializeField] private RayRange _raysUp, _raysRight, _raysDown, _raysLeft;
    [SerializeField] private bool _colUp, _colRight, _colDown, _colLeft;

    [SerializeField] private float _timeLeftGrounded;
